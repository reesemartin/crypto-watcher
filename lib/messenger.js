...
const sendPriceIntervalAction = async () => {
  ...
  const newPrice = Number(price.split(' ')[2])
  ...
  else if (firstRun || Math.abs(priceChange) > 0.02) {
    lastPrice = newPrice
    sendPrice(process.env.TELEGRAM_CHAT_ID)(`Price change notice: ${price} ${priceChange > 0 ? 'up' : 'down'} ${priceChange} from ${lastPrice}`)
  }
}
